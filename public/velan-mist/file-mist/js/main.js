...
function windowLoadInit() {
	...
	$("#demo .accordion__ul .accordion__li").hover(
		...
		$('.wc-tabs li button, ul.tabs li button').on( 'click', function( e ) {
			e.preventDefault();
			var $tab          = $( this );
		...
		$('a.woocommerce-review-link').on( 'click', function() {
			$( '.reviews_tab button' ).trigger('click');
			return true;
		});
		...
		if ( hash.toLowerCase().indexOf( 'comment-' ) >= 0 || hash === '#reviews' || hash === '#tab-reviews' ) {
			$tabs.find( 'li.reviews_tab button' ).trigger('click');
		} else if ( url.indexOf( 'comment-page-' ) > 0 || url.indexOf( 'cpage=' ) > 0 ) {
			$tabs.find( 'li.reviews_tab button' ).trigger('click');
		} else if ( hash === '#tab-additional_information' ) {
			$tabs.find( 'li.additional_information_tab button' ).trigger('click');
		} else {
			$tabs.find( 'li:first button' ).trigger('click');
		}
...
// Our Products
function openCatalog(image){
    document.getElementById('catalogImage').src = image;
    document.getElementById('catalogPopup').style.display = 'flex';
}
...
document.getElementById('catalogPopup').addEventListener('click', function(e){
    if(e.target === this){
        closeCatalog();
    }
});
